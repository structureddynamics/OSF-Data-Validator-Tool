  ...
  */  
  ...
  class CheckOwlRestrictionSome extends Check
  {
    ...
    public function run()
    {
            ...
            foreach($exists as $existential)
            {
              ...
              if(!empty($existential['dataRange']))
              {
                ...
                else
                {
                  ...
                  {
                    if($customDatatypes[$existential['dataRange']] === 'http://www.w3.org/2001/XMLSchema#anySimpleType')
                    {
                      $datatypeFilter = 'filter(str(datatype(?value)) = \''.$existential['dataRange'].'\' || str(datatype(?value)) = \''.$customDatatypes[$existential['dataRange']].'\'  || str(datatype(?value)) = \'http://www.w3.org/2001/XMLSchema#string\')';
                    }
                    else
                    {
      ...
      {
        cecho("We couldn't get the list of existential restriction from the OSF Web Services instance\n", 'YELLOW');
      ...
      {
        cecho("We couldn't get the number of object properties per record from the OSF Web Services instance\n", 'YELLOW');
